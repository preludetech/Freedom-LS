...
- **The completion hook only fires on a None -> set transition.** It is a `post_save` receiver in `freedom_ls/student_progress/signals.py`; `CourseItemProgress.__init__` snapshots `_original_completion_value` and `newly_completed_item()` reports the transition, so creating a `TopicProgress(complete_time=now)` or passing `completed_time=now` to a factory means original==current at save time and the auto-update of `CourseProgress` does not run. To trigger it, create the row first, then assign `complete_time` and call `.save()`. Or call `update_course_progress_on_completion(user, item)` directly afterwards. `queryset.update()` and `bulk_create()` never fire it.

- **`update_course_progress_on_completion` does not set `site` when it creates the `CourseProgress`** (`freedom_ls/learner_progress/signals.py`), so an `update_or_create` from cold with no pre-existing row will hit a `NotNullViolation` on `site_id`. Workaround for QA seeding: pre-create `CourseProgress` yourself with `site=site` and the calculated percentage (use `freedom_ls.learner_management.utils.calculate_course_progress_percentage`), then set `completed_time` and save.
