...
- [reference_qa_run_residue_cleanup.md](reference_qa_run_residue_cleanup.md) — Tearing down a manual QA run's residue (signup user + LegalConsent x2 + EmailAddress, CourseInterest, a self-service enrolment); Collector.fast_deletes hides half the cascade; CourseProgress PROTECTs LearnerCourseRegistration; also secondary EmailAddress rows on a KEPT user (no cascade, EmailConfirmation always 0); the security-guard hook fails with an EMPTY error and matches its denylist anywhere in the command
...
- [reference_coming_soon_section_fixtures.md](reference_coming_soon_section_fixtures.md) — qa_create_coming_soon_sections: the MIX/UNCAT/HIDDEN/ONLY coming-soon dashboard seeds; a coming_soon course with no/hidden category renders ONLY in Coming soon (never the catch-all); the "Free" chip is anonymous-only; demo category slugs are `*-demo`
...
**Sixth shape, same branch: "just move this category's `order`".** One-field tweaks on an existing QA fixture (`qa-soon-only` order 0 -> 60) are their own ask now. Recipe: locate + print, `assert` the prior value, `queryset.update(order=...)`, re-read and print, report old AND new. The gotcha is that the seeding command's `update_or_create` `defaults` hold the ORIGINAL order, so a re-run of `qa_create_coming_soon_sections` reverts the tweak — warn about it every time. See [[reference_coming_soon_section_fixtures]].
