---
name: course-player-qa-learner-command
description: Management command that seeds a login-ready learner exercising the three course-player redirect/resume cases
metadata:
  type: reference
---

`uv run python manage.py qa_create_course_player_learner [SITE_NAME]` (default `DemoDev`) seeds a single login-ready learner for QA of the learner course player. Command file: `freedom_ls/qa_helpers/management/commands/qa_create_course_player_learner.py`. Idempotent.

It creates learner `demodev_s1@email.com` (password == email, the project login convention; the `UserFactory` already defaults password to email) with a verified+primary allauth `EmailAddress` (required because `ACCOUNT_EMAIL_VERIFICATION = "mandatory"`), and sets up:
...
Note: there is no separate LearnerProfile model — a "learner" is just a `User` plus registrations. See [[reference_verified_learner_setup]].
