...
- `DeleteAction.has_permission` → object-level `request.user.has_perm("freedom_ls_learner_management.delete_cohort", cohort)`. Grant with `assign_perm("delete_cohort", educator, cohort)` (guardian).
...
Important: `Cohort._meta.app_label == "freedom_ls_learner_management"` (set via `apps.py` `label =`), NOT `learner_management`. The full perm string is `freedom_ls_learner_management.delete_cohort`.
...
Command `qa_create_educator_modal_target <site_name>` (in qa_helpers) builds the minimal setup: login-ready educator (`qa_educator@example.com`, password == email, verified+primary EmailAddress), a `QA Modal Cohort` with view+delete perms, one learner member, and a course registration (for the modal's cascade summary). Idempotent.
