...
class TestLoadRoster:
    def test_students_are_ordered_by_surname(self, mock_site_context):
        cohort = CohortFactory()
        ...
        assert roster.student_ids == [alpha.id, zeta.id]

    def test_a_student_with_no_surname_is_keyed_by_their_email(self, mock_site_context):
        cohort = CohortFactory()
    ...
    @override_settings(REPORTS_MAX_STUDENTS=1)
    ...
    @override_settings(REPORTS_MAX_STUDENTS=2)
    def test_a_cohort_exactly_at_the_limit_is_allowed(self, mock_site_context):
        cohort = CohortFactory()
        CohortMembershipFactory(cohort=cohort, user=UserFactory())
        CohortMembershipFactory(cohort=cohort, user=UserFactory())

        assert len(load_roster(cohort, mock_site_context.pk).student_ids) == 2
    ...
    def test_members_and_their_users_are_read_in_one_query(
        ...
        with django_assert_num_queries(1):
            roster = load_roster(cohort, mock_site_context.pk)
            [user.display_name for user in roster.students_by_id.values()]
...
class TestLoadTopicProgressRows:
    def test_only_the_named_students_rows_are_read(self, mock_site_context):
        student, other = UserFactory(), UserFactory()
        topic = TopicFactory()
        TopicProgressFactory(user=student, topic=topic, complete_time=timezone.now())
        TopicProgressFactory(user=other, topic=topic, complete_time=timezone.now())

        rows = load_topic_progress_rows(mock_site_context.pk, [student.id], {topic.id})

        assert [row[0] for row in rows] == [student.id]
    ...
    def test_rows_are_read_in_one_query(
        ...
        with django_assert_num_queries(1):
            load_topic_progress_rows(mock_site_context.pk, [student.id], {topic.id})
...
class TestLoadFormProgressRows:
    def test_completed_sittings_arrive_newest_first(self, mock_site_context):
        ...
        with time_machine.travel("2026-01-01T00:00:00Z", tick=False):
            older = FormProgressFactory(
                user=student, form=quiz, completed_time=timezone.now(), scores={}
            )
        ...
        rows = load_form_progress_rows(mock_site_context.pk, [student.id], {quiz.id})
    ...
    def test_an_incomplete_sitting_sorts_after_every_completed_one(
        ...
        student = UserFactory()
        quiz = FormFactory(strategy=FormStrategy.QUIZ)
        started = FormProgressFactory(user=student, form=quiz, completed_time=None)
    ...
    def test_reading_each_rows_form_issues_no_further_query(
        ...
        with django_assert_num_queries(1):
            rows = load_form_progress_rows(
                mock_site_context.pk, [student.id], {quiz.id}
            )
            [row.form.quiz_pass_percentage for row in rows]


class TestLoadFirstAttemptIds:
    def test_only_the_earliest_completed_sitting_per_student_and_quiz_is_returned(
        self, mock_site_context
    ):
        student = UserFactory()
        quiz = FormFactory(strategy=FormStrategy.QUIZ)
    ...
    def test_an_incomplete_sitting_is_never_a_first_attempt(self, mock_site_context):
        student = UserFactory()
        quiz = FormFactory(strategy=FormStrategy.QUIZ)
        FormProgressFactory(user=student, form=quiz, completed_time=None)

        assert (
            load_first_attempt_ids(mock_site_context.pk, [student.id], {quiz.id})
            == set()
        )

    def test_two_students_each_contribute_their_own_first_attempt(
        self, mock_site_context
    ):
        first_student, second_student = UserFactory(), UserFactory()
        quiz = FormFactory(strategy=FormStrategy.QUIZ)
        one = FormProgressFactory(
            user=first_student, form=quiz, completed_time=timezone.now(), scores={}
        )
        two = FormProgressFactory(
            user=second_student, form=quiz, completed_time=timezone.now(), scores={}
        )

        attempt_ids = load_first_attempt_ids(
            mock_site_context.pk, [first_student.id, second_student.id], {quiz.id}
        )
...
class TestLoadSelectedOptionsByPair:
    def test_a_survey_answer_is_excluded(self, mock_site_context):
        ...
        sitting = FormProgressFactory(
            user=student, form=survey, completed_time=timezone.now(), scores={}
        )
        QuestionAnswerFactory(
...
def _cohort_with_one_completed_quiz(*, student_count: int):
    """One course, one topic and one two-question quiz every student has sat."""
    cohort = CohortFactory()
    course = CourseFactory()
    CohortCourseRegistrationFactory(cohort=cohort, collection=course)
    topic = TopicFactory()
...
class TestTheAssemblyStageIssuesNoQueriesOnRealRows:
    ...
    def test_building_course_sections_from_loaded_rows_issues_no_queries(
        ...
        cohort = _cohort_with_one_completed_quiz(student_count=2)
        ...
        assert [row.completion_percentage for row in sections[0].student_rows] == [
            100,
            100,
        ]

    def test_building_student_details_from_loaded_rows_issues_no_queries(
        self, mock_site_context, django_assert_num_queries
    ):
        cohort = _cohort_with_one_completed_quiz(student_count=2)
        loaded = _load_everything(cohort, mock_site_context.pk)
        ...
        with django_assert_num_queries(0):
            details = [
                ...
                for user_id in loaded["roster"].student_ids
            ]
