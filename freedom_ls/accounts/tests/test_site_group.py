...
class TestSiteGroupIsolation:
    ...
    def test_same_group_name_different_sites_allowed(self, site_a, site_b):
        ...
        # Set unique names for the internal 'name' field to avoid unique constraint
        group_a = SiteGroup(group_name="Teachers", site=site_a)
        group_a.name = f"Teachers ({site_a.domain})"
        group_a.save()
        
        group_b = SiteGroup(group_name="Teachers", site=site_b)
        group_b.name = f"Teachers ({site_b.domain})"
        group_b.save()
    ...
    def test_group_queryset_filters_by_current_site(self, site_a, site_b, mock_site_a_context):
        ...
        # Create groups on both sites with unique names
        group_a = SiteGroup(group_name="SiteA Group", site=site_a)
        group_a.name = f"SiteA Group ({site_a.domain})"
        group_a.save()
        
        group_b = SiteGroup(group_name="SiteB Group", site=site_b)
        group_b.name = f"SiteB Group ({site_b.domain})"
        group_b.save()
...
class TestSiteGroupGuardianIntegration:
    ...
    def test_assign_object_permission_to_site_group(self, site_a, mock_site_a_context):
        ...
        group = SiteGroup(group_name="Editors", site=site_a)
        group.name = f"Editors ({site_a.domain})"
        group.save()
    ...
    def test_object_permissions_isolated_between_sites(self, site_a, site_b):
        ...
        # Create groups on both sites with unique names
        group_a = SiteGroup(group_name="Editors", site=site_a)
        group_a.name = f"Editors ({site_a.domain})"
        group_a.save()
        
        group_b = SiteGroup(group_name="Editors", site=site_b)
        group_b.name = f"Editors ({site_b.domain})"
        group_b.save()
    ...
    def test_user_inherits_group_permissions(self, site_a, mock_site_a_context):
        ...
        group = SiteGroup(group_name="Teachers", site=site_a)
        group.name = f"Teachers ({site_a.domain})"
        group.save()
    ...
    def test_site_group_object_permission_uses_custom_model(self, site_a):
        ...
        group = SiteGroup(group_name="Admins", site=site_a)
        group.name = f"Admins ({site_a.domain})"
        group.save()
...
class TestSiteGroupModel:
    ...
    def test_site_group_auto_sets_site_from_context(self, mock_site_a_context, site_a):
        ...
        group = SiteGroup.objects.create(group_name="Auto Site Group")
    ...
    def test_multiple_groups_can_be_created_on_same_site(self, site_a, mock_site_a_context):
        ...
        group1 = SiteGroup(group_name="Teachers", site=site_a)
        group1.name = f"Teachers ({site_a.domain})"
        group1.save()
        
        group2 = SiteGroup(group_name="Students", site=site_a)
        group2.name = f"Students ({site_a.domain})"
        group2.save()
        
        group3 = SiteGroup(group_name="Admins", site=site_a)
        group3.name = f"Admins ({site_a.domain})"
        group3.save()
...
class TestUserSiteGroupIntegration:
    ...
    def test_user_can_be_in_multiple_site_groups(self, site_a, mock_site_a_context):
        ...
        group2 = SiteGroup(group_name="Admins", site=site_a)
        group2.name = f"Admins ({site_a.domain})"
        group2.save()
    ...
    def test_user_from_site_a_cannot_join_site_b_group(self, site_a, site_b, mock_site_a_context):
        """A user from one site should not be able to join a group from another site."""
        ...
        # This should technically work at the database level, but in practice
        # the admin and views should prevent this. However, we can test that
        # they exist on different sites
