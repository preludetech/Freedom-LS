...
from conftest import site, mock_site_context
...
class TestSiteGroupIsolation:
    ...
    def test_groups_created_on_site_a_not_visible_on_site_b(self, site, site_b, mock_site_context):
        """Groups created on site A should not be visible when querying from site B."""
        # Create a group on site A
        group_a = SiteGroup.objects.create(group_name="Teachers", site=site)
        group_b = SiteGroup.objects.create(group_name="Teachers", site=site_b)

        groups = list(
            SiteGroup.objects.all())  # this will fetch all the groups available on site=site since the
        # mock_site_context is already set up to patch the request

        assert groups == [group_a]

    def test_same_group_name_different_sites_allowed(self, site, site_b):
    ...
    def test_duplicate_group_name_same_site_not_allowed(self, site):
        """Same group name cannot exist twice on the same site."""
        SiteGroup.objects.create(group_name="Teachers", site=site)

        with pytest.raises(IntegrityError):
            SiteGroup.objects.create(group_name="Teachers", site=site)

    def test_group_queryset_filters_by_current_site(self, site, site_b, mock_site_context):
        """SiteGroup.objects.all() should only return groups for the current site."""
        # Create groups on both sites
        group_a = SiteGroup.objects.create(group_name="SiteA Group", site=site)
...
class TestSiteGroupGuardianIntegration:
    ...
    def test_assign_object_permission_to_site_group(self, site, mock_site_context):
        """Object permissions can be assigned to a SiteGroup."""
        group = SiteGroup.objects.create(group_name="Editors", site=site)
        activity = Activity.objects.create(title="Test Activity", slug="test-activity", site=site)
    ...
    def test_object_permissions_isolated_between_sites(self, site, site_b):
        """Object permissions on site A groups should not be accessible from site B."""
        # Create groups on both sites
        group_a = SiteGroup.objects.create(group_name="Editors", site=site)
        group_b = SiteGroup.objects.create(group_name="Editors", site=site_b)

        # Create activity on site A
        activity_a = Activity.objects.create(title="Activity A", slug="activity-a", site=site)
    ...
    def test_user_inherits_group_permissions(self, site, mock_site_context):
        """Users assigned to a SiteGroup should inherit object permissions."""
        group = SiteGroup.objects.create(group_name="Teachers", site=site)
        user = User.objects.create_user(email="teacher@example.com", password="pass", is_active=True)
        activity = Activity.objects.create(title="Lesson 1", slug="lesson-1", site=site)
    ...
    def test_remove_user_from_group_removes_permissions(self, site, mock_site_context):
    ...
    def test_site_group_object_permission_uses_custom_model(self, site):
        """Verify that SiteGroupObjectPermission is used for storing group permissions."""
        group = SiteGroup.objects.create(group_name="Admins", site=site)
        activity = Activity.objects.create(title="Test", slug="test", site=site)
...
class TestSiteGroupModel:
    ...
    def test_site_group_str_representation(self, site):
        """Test string representation of SiteGroup."""
        group = SiteGroup.objects.create(group_name="Moderators", site=site)
        assert str(group) == "Moderators"

    def test_site_group_auto_sets_site_from_context(self, mock_site_context, site):
        ...
        group = SiteGroup.objects.create(group_name="Auto Site Group", site=site)
        assert group.site == site


    def test_site_group_inherits_from_site_aware_base(self, site):
        """Verify SiteGroup inherits from SiteAwareModelBase."""
        group = SiteGroup.objects.create(group_name="Test", site=site)
        assert hasattr(group, "site")
        assert group.site == site

    def test_multiple_groups_can_be_created_on_same_site(self, site, mock_site_context):
        """Multiple groups with different names can exist on the same site."""
        group1 = SiteGroup.objects.create(group_name="Teachers", site=site)
        group2 = SiteGroup.objects.create(group_name="Students", site=site)
        group3 = SiteGroup.objects.create(group_name="Admins", site=site)

        assert SiteGroup.objects.count() == 3
        assert all(g.site == site for g in [group1, group2, group3])
...
class TestUserSiteGroupIntegration:
    ...
    def test_user_groups_field_points_to_site_group(self, site, mock_site_context):
        ...
        group = SiteGroup.objects.create(group_name="TestGroup", site=site)
    ...
    def test_user_can_be_in_multiple_site_groups(self, site, mock_site_context):
        """A user can belong to multiple SiteGroups on the same site."""
        user = User.objects.create_user(email="user@example.com", password="pass", is_active=True)

        group1 = SiteGroup.objects.create(group_name="Teachers", site=site)
        group2 = SiteGroup.objects.create(group_name="Admins", site=site)
    ...
    def test_cross_site_group_assignment_creates_inconsistency(self, site, site_b, mock_site_context):
