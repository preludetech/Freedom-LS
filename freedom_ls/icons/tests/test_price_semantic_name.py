"""Tests verifying the 'price' semantic icon is wired across all icon sets."""
...
def test_price_is_a_semantic_name() -> None:
    assert "price" in SEMANTIC_ICON_NAMES


@pytest.mark.parametrize("set_name", list(ICON_SETS.keys()))
def test_price_is_mapped_in_every_set(set_name: str) -> None:
    config = ICON_SETS[set_name]
    assert "price" in config.mapping, (
        f"icon set {set_name!r} is missing a mapping for 'price'"
    )


@pytest.mark.parametrize("set_name", list(ICON_SETS.keys()))
def test_price_glyph_and_variants_resolve(set_name: str) -> None:
    config = ICON_SETS[set_name]
    glyph = config.mapping["price"]
