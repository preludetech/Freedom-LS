...
- **Object storage for media** — media is served from S3-compatible object storage (Cloudflare R2), split across three buckets by sensitivity: one for anonymously readable branding, one for course content, one for content tied to an identified learner. Each of six named locations resolves independently, so a deployment can point every one at its own bucket or leave any of them unset and fall back to local filesystem storage. Media is **private by default**, with time-limited signed links rather than permanently public URLs. See [security and data handling](./security-and-data-handling.md).
- **Cohort progress reports** — generates a per-cohort progress report as a PDF. Generation always runs as a background task and always produces a stored file, never a public media file: reports hold real learner names and quiz answers, so they are written to a private storage location configured separately from ordinary media. A deployment that has not configured one now fails to start rather than falling back to shared media storage, and a deploy pipeline that runs Django's deployment check catches a misconfigured location before anything is written to it. See [security and data handling](./security-and-data-handling.md). The report bundles its own fonts, so rendering does not depend on what the base image happens to carry.
