...
- **Analytics (Google Analytics 4)** — the `freedom_ls.google_tag` app: a client-side snippet configured by `GOOGLE_ANALYTICS_MEASUREMENT_ID`. A project that wants neither GA4 nor Google Ads leaves the app out of `INSTALLED_APPS` and drops its context processor; events its views record are then discarded. With no ID set nothing loads and nothing is sent. Signed-in sessions carry the numeric account ID as `user_id`, never an email address. Neither analytics snippet loads on the email-confirmation or password-reset pages, whose URLs carry one-time tokens. For visitors in the EEA, the UK and Switzerland the tag sets Google Consent Mode to denied, so GA4 and Google Ads send cookieless pings there; FLS ships no banner, and a deployment that adds one grants consent itself. The GA4 property needs one-off setup before its reports are useful. See [Google Analytics 4 setup](#google-analytics-4-setup).
...
The IDs are per deployment, so every site a deployment serves shares one GA4 property and one Ads account; reports tell the sites apart by hostname. Staging gets its own GA4 property and no Google Ads settings, or test sign-ups count as real Ads conversions.
...
1. Under Admin, Custom definitions, register seven event-scoped custom dimensions: `course_id`, `course_slug`, `access_type`, `request_kind`, `registration_method`, `registration_source` and `lead_form`. `course_id` is the course's UUID and never changes, while a slug can be renamed and split a course's history across two values, so report on `course_id` wherever history matters.
2. Count applications apart from interest: under Admin, Events, Create event, match `course_access_requested` where `request_kind` equals `application` and name the result `course_application_submitted`.
3. Mark five key events: `sign_up`, `course_registered`, `course_application_submitted`, `course_completed` and `generate_lead`. Leave `course_access_requested` unmarked, or each application counts twice and every interest click counts as a conversion.
4. Property settings: reporting identity Blended or Observed (otherwise `user_id` has no effect), event data retention 2 months, Google Signals off, user-provided data collection off, email and query-parameter redaction on, and an internal-traffic filter for the operator's own team.
5. In the data stream's enhanced measurement settings, leave "Page changes based on browser history events" on. FLS relies on it for page views during in-page navigation. Consider turning "Form interactions" off. It fires for every form on the site, quiz pages included.
6. Build the funnel under Explore, Funnel exploration, with the steps `sign_up`, then `course_access_requested` or `course_registered`, then `course_started`, then `course_completed`. Break it down by `course_id` or `access_type`.
...
Setting `GOOGLE_ADS_CONVERSION_ID` to the account's `AW-` ID adds Google Ads as a second destination of the tag GA4 already loads. Ads then sets its click and conversion cookies on every page and can build remarketing audiences. It loads through the GA4 tag, so it needs `GOOGLE_ANALYTICS_MEASUREMENT_ID` as well; an Ads ID on its own produces warning `freedom_ls_google_tag.W001` and loads nothing. The Ads tag is off wherever the GA4 tag is off, including the two token-bearing pages.
...
**Native Ads conversions.** In Google Ads open Goals, Conversions, New conversion action, Website, and add the action manually. Its tag snippet ends in `send_to: 'AW-XXXXXXXXX/LABEL'`; the part after the slash is the label. Map event names to labels in `GOOGLE_ADS_CONVERSION_LABELS`, for example `sign_up=AbCdEfGhIj,course_registered=KlMnOpQrSt`. A mapped event then sends `gtag('event', 'conversion', {send_to: ...})` in the same script as its GA4 event, so the two fire together, once, wherever the GA4 event fires. An event with no label sends no conversion. Labels without an Ads ID produce warning `freedom_ls_google_tag.W002`. Any event name works as a key, including a downstream project's own events, so a lead form can report `generate_lead` to Ads the same way. A malformed entry fails the application at boot.

Only one kind of conversion should drive bidding. Make `course_registered`, `course_application_submitted` and `generate_lead` primary, and `sign_up` and `course_completed` secondary; a learner who signs up and then registers would otherwise count twice, and bidding would chase sign-ups that never become learners. Leave enhanced conversions off, since FLS sends Google no email addresses.
