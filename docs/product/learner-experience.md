...
- A form question can ask for text, a number, a date, a time, an email address, a URL, a phone number, a file upload, or a choice made from radio buttons, checkboxes or a dropdown. An answer that does not fit its question's type is refused at submission, with the reason shown on the question itself, rather than stored as typed.
...
Form items can span multiple pages. A learner can leave a form part-way through and resume later with their answers preserved. Leaving is only guarded by a confirmation prompt when there are unsaved changes to lose. A question marked required must be answered before the learner can move on. Resuming lands the learner on the furthest page they have reached, so a skipped optional question on an earlier page never sends a returning learner backwards. A course form offers the same question types as an [application form](#applying-to-a-course), and refuses an answer that does not fit its type the same way.
