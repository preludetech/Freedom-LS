...
- [ ] Where email queueing is turned on, dumps are treated as carrying live credentials:
      the message waits in the database, so a dump taken inside the prune window contains
      unexpired password-reset and signup-verification links. See
