...
organisation-scoped** — precisely the blast radius the since-deleted `critical_security_fixes` spec described ("Site
...
3. **Make `get_filters` request-aware**, or better, fold it into a single
   ...
   (See `../educator-interface-1-panel-framework-core/research_panel_framework_surface.md` §3 — this weakness exists today, regardless.)
