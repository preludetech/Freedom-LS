...
> **Superseded points.** The idea beside this file, and `../educator-interface-full-polish/htmx-modal-drawer-url-state.md`,
> were written after this survey and win where they disagree. Four points below are overridden:
> click-outside does *not* dismiss on desktop, because clicking the next row is the point; the drawer is a
> native `<dialog>` opened with `show()`, not a `role="complementary"` region; focus stays on the trigger
> after opening rather than moving to the panel header; and the mobile flip happens at Tailwind `md`, with no
> separate overlay mode at 1024px. The survey itself still stands.
