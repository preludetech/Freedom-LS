...
- **Quick-view drawer**: a drawer that slides in from the right without a backdrop, so the main
  ...
  It is a `role="region"` on desktop and a `role="dialog"` sheet below `md`. It is built by
  `../educator-interface-3-panel-framework-dialogs/`, which decided on a native `<dialog>` rather than these roles.
...
Two known table problems are fixed by
`../educator-interface-2-panel-framework-tables/`: sibling tables on one
