...
| New template(s) reusing `course_form_page.html`'s partials, wrapped in a non-course base | `course_form_page.html`'s body content is already reusable; needs a base template with no course TOC in place of `_exam_runner_base.html`'s (which is already course-free — may be directly reusable) vs. `_course_base.html` (not) | **Must** — §4 |
...
| `freedom_ls/learner_interface/*` | None | **No change** — `_course_access_redirect`, `form_fill_page`, `view_form` etc. stay exactly as they are; the application flow does not run through them |
