...
Parent context: see `spec_dd/3. done/2026-04-28_07:25_testing-best-practice/1. spec.md` (Phase 1) for the shared decisions, skip list, and parallelism map. Concrete file:line citations live in `spec_dd/3. done/2026-04-28_07:25_testing-best-practice/research_audit.md`.
...
The audit identified **~12 CSS-class assertion sites, ~2 hardcoded-config-value sites, and ~6 trivial-model-default sites**. The exact citations are in `spec_dd/3. done/2026-04-28_07:25_testing-best-practice/research_audit.md` §1, §6, §7. Spec author should re-verify counts at implementation time (the audit was a snapshot).
...
- No edits to `SKILL.md` files. The targeted addition to `${CLAUDE_PLUGIN_ROOT}/resources/testing.md` documenting the `transaction=True` rule (described above) is in scope; no other resource doc rewrites.
...
1. All CSS-class assertion sites identified by the audit §1 are either deleted or rewritten to assert on information (text, accessible label, presence-by-role).
2. All hardcoded-config-value assertion sites identified by the audit §6 are rewritten to assert on structure / invariants (or deleted with rationale in the PR description).
3. All trivial model-default tests identified by the audit §7 are deleted.
...
7. No production code changes. No `SKILL.md` edits (the targeted `resources/testing.md` `transaction=True` rule is the only doc addition allowed). No new factories. No new view-assertion work.
