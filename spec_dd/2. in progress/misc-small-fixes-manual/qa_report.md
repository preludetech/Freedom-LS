...
The pass was driven manually through Playwright MCP against a dev server on port 8623. Screenshots
were collected into `screenshots/` beside this report, and every image referenced below exists there.
...
organisation.is_default`. It reads "Powered by FirstClass" for any other organisation. Anyone
re-running this check should pick a non-house cohort, or the missing tenant name will look like a
...
status: ok · reason: 1 bug — 0 fixed, 1 unresolved (red lane: turns on a product decision); 176 checks run, 175 passed; report rendered, screenshots verified
