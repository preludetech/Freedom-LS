...
- [ ] (user + cmd) Reconcile the two implementations of "is this form placement done": the course outline reads the latest-**started** attempt via `quiz_verdict` (`freedom_ls/learner_interface/utils.py` `get_content_status`), while the progress percentage and the finish page's `outstanding_items` read the latest-**completed** attempt via `attempt_completes_form` (`freedom_ls/learner_progress/queries.py`). A learner who starts a fresh attempt without finishing it can make the outline and the finish page disagree. Pre-existing and unrelated to the B2 fix, which deliberately used the percentage's source so those two agree
