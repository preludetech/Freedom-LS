...
- [ ] (user + cmd) Fix: a `<c-content-link>` that resolves to a Form renders `<a href="">`. `Form` has no `preview_url`, unlike `Topic` (`freedom_ls/content_engine/models/topics.py:18`), so `content-link.html:12` emits an empty href. Surfaced by the demo-content Form link added for the item above, and recorded as a known non-finding in QA plan `3c`'s §0.4
- [ ] (user + cmd) Fix: `Topic.preview_url()` reverses `content_engine:topic_detail`, which no URLconf defines — every `topic_detail` route is commented out at `freedom_ls/learner_interface/urls.py:52-62`. Any `<c-content-link>` that resolves to a Topic raises `NoReverseMatch` and 500s. Latent today only because no demo content links to a resolvable Topic; the three existing links all point at `01-what-is-git-for.md`, which exists nowhere. Belongs with the item above, in the content-link spec that the `TODO. - non-preview link` in `content-link.html:5` already names — leave that TODO in place
- [ ] (cmd) Fix the order-dependent flake in `freedom_ls/accounts/tests/test_allauth_signup_policy.py::test_is_open_for_signup_respects_force_site_name`. Under `pytest-randomly` it intermittently asserts `True is False` — `is_open_for_signup` falls back to the global `ALLOW_SIGN_UPS` instead of the ForcedSite policy — while passing in isolation, in deterministic order (`-p no:randomly`), and across 6/6 narrow runs. Confirmed pre-existing, not a regression from this branch: it fails on full-suite runs that exclude every file touched by the content-link work above
