...
- [ ] (user) Decide whether completing a form attempt should refresh its record's `last_accessed_time`. QA run `3a. seam_qa` bug B2: the field currently tracks the last item VIEW, not the last submission (observed 137s stale immediately after a passing sitting). `recalculate_progress_percentage` in `freedom_ls/learner_progress/signals.py` saves with `update_fields=["progress_percentage"]` and its comment says a background recalculation must deliberately not look like a visit — so this may be correct as-is. If it is, soften plan `3a. seam_qa/frontend_qa_seam.md` S1.5, which asserts `last_accessed_time` is "within the last minute" after completing a form
...
- [ ] (user + cmd) Review the uncommitted new file `freedom_ls/qa_helpers/management/commands/qa_grant_organisation_staff.py`, created during QA run `3a. seam_qa` by the `fls-dev:qa-data-helper` agent to grant a user `organisation_staff` on an organisation and optionally reset their password. It is untested by the suite — either commit it deliberately with tests or delete it. The same agent also reset `org.educator@example.com`'s password to `demodev@email.com` and granted Olive `organisation_staff` on the DemoDev organisation, which widened her visible DemoDev cohort list from 1 to 4
- [ ] (user) Delete `LearnerDeadline` 53ff5b27 before reusing Cara Learner as a clean fixture — QA run `3a. seam_qa` S10.6 left a hard past deadline (2026-08-20) on her Northside individual registration for the Knowledge Check, which locks her out of that item whenever she resolves to her Northside grant
- [ ] (user) Re-run `uv run python manage.py danger_content_delete` and confirm it completes without a `ProtectedError`, then re-seed from `3a. seam_qa/frontend_qa_seam.md` §0.1. QA run `3a. seam_qa` did not run S8 (the plan designates it HUMAN-RUN) and left fresh residue for it to clear: two deliberately orphaned `CourseFormAttempt` rows (8cf50321, e9d81dd3, both `collection_item=None`) and three hand-built `FormProgress` rows (222d5c54, bfd56e9c, 5756c9fc)
