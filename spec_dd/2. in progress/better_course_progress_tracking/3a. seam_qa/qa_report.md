...
| S8 | ✅ PASS | Not executed inside this run (the plan then marked it HUMAN-RUN). Run out-of-band on 2026-08-27: 200 content objects cleared, no `ProtectedError`, §0.1 re-seeded clean. |
...
**S8 was not run inside this pass**, because the plan then marked it HUMAN-RUN and running it would have destroyed the fixtures the rest of the run depended on. It was run out-of-band on 2026-08-27: `danger_content_delete --yes` cleared 200 content objects with **no `ProtectedError`**, and §0.1 re-seeded clean afterwards. The HUMAN-RUN flag has since been removed from the plan — the step never needed human judgement, only the `--yes` flag, since bare it blocks forever on a confirmation prompt.
...
- **`danger_content_delete` completes without a `ProtectedError`.** Met — verified out-of-band on 2026-08-27 rather than inside this pass: 200 content objects deleted, no `ProtectedError`, the `QuestionAnswer` → `CourseFormAttempt` → `FormProgress` → `TopicProgress` → `CourseProgress` chain cleared ahead of the content as intended.
