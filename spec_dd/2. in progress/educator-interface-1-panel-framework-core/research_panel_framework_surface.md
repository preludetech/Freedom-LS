...
- `educator_interface/views.py:1112-1115` — a `@claude` note that `CourseDataTable` shows every
  course on the site to every logged-in user, deferred to a `critical_security_fixes` spec that has since been deleted. Spec 6 (cohort administration) closes it by rebuilding the courses section.
