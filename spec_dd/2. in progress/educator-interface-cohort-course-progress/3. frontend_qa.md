...
All commands require `--site-domain 127.0.0.1:8000` (adjust if different).

**Create a cohort with 25+ students** (for Test 11 — row pagination):
```
uv run python manage.py qa_create_large_cohort --site-domain 127.0.0.1:8000 --course-slug functionality-demo-show-end-with-topic
```

**Create a cohort with registrations but no students** (for Test 3 — empty state):
```
uv run python manage.py qa_create_empty_student_cohort --site-domain 127.0.0.1:8000 --course-slug functionality-demo-show-end-with-topic
```

**Complete a non-quiz form for cohort students** (for Test 9):
```
uv run python manage.py qa_complete_form --site-domain 127.0.0.1:8000 --cohort-name "Demo Cohort" --form-slug course-feedback-survey
```

**Create a soft deadline on an item** (for Test 14 — soft vs hard overdue):
```
uv run python manage.py qa_create_soft_deadline --site-domain 127.0.0.1:8000 --cohort-name "Demo Cohort" --course-slug functionality-demo-show-end-with-topic --item-slug pictures
```

**Create a student deadline override** (for Test 13):
```
uv run python manage.py qa_create_deadline_overrides --site-domain 127.0.0.1:8000 --cohort-name "Demo Cohort" --course-slug functionality-demo-show-end-with-topic --student-email demodev@email.com --item-slug pictures --days-from-now 14
```
