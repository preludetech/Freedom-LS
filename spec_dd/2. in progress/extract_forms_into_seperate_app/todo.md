...
- [x] (user + cmd) Fix QA bug: `quiz_percentage()` KeyErrors on a completed QUIZ attempt whose scores dict has no 'score' key, breaking `recalculate_progress_percentages` and cohort report generation — pre-existing on main, not a regression (TDD — failing test first, then fix)
- [x] (user + cmd) Fix QA bug: 'Leave and submit' on a submit_on_exit form discards the current page's answers, locking in a 0% failed attempt — pre-existing on main, not a regression (TDD — failing test first, then fix)
