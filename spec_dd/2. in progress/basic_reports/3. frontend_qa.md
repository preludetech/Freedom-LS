...
| `no-pass-mark-cohort` | 8–10 students | medium, **one quiz with `quiz_pass_percentage` unset** | Score without verdict must render, not crash or vanish (QA 2.8, QA 12.2) |
...
- A quiz whose `quiz_pass_percentage` is left unset (`None`) — see below.
