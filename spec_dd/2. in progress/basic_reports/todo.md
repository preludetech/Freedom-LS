...
- [x] (user + cmd) Fix QA bug: the report's per-learner "Incorrect answers" table paints every option the learner ticked as an error, so on a checkbox question the correctly-ticked options are red under "Answers given" and green under "Correct answer" on the same row — `wrong_selected_counts` in `reports/gather.py` never filters on `option.correct`, and `student_detail.html:106` renders them all as `chip-error`; the cohort confusions path already does this correctly via `.exclude(correct=True)`, and the student results page is the reference implementation (TDD — failing test first, then fix)
- [x] (user + cmd) Fix QA bug: a course item shown as "Locked" in the course index is not enforced server-side — `view_course_item` gates only on hidden-course, `can_access_content` and deadline locks, so a learner who fails a gating quiz can open the locked item by URL and get its full content; the visit also writes `last_accessed_item`, permanently flipping the index entry from "Locked" to "In progress" with a live link (TDD — failing test first, then fix)
...
- [x] (cmd) Re-run `/fls-dev:do_qa` on `3a. report_generation_qa/frontend_qa_report_generation.md` for the organisation changes — QA 2.1 and QA 8 both changed, and QA 8.8–8.11 are new
- [ ] (user + cmd) Decide QA 8.9: a foreign-organisation user gets a 302 to the admin index (Django's default out-of-queryset behaviour, leaking nothing) where the plan specifies a 404 — either raise `Http404` in the admin's `get_object` or soften the plan's wording (TDD — failing test first, then fix)
- [ ] (user + cmd) Fix QA bug: one missing font file emits four identical `freedom_ls_reports.W004` warnings because the Inter family declares four faces pointing at the same variable file — deduplicate by `static_path` before reporting (TDD — failing test first, then fix)
- [ ] (user + cmd) Fix QA bug: the confusions "Got it wrong" column renders "1 of 1 learners" instead of "1 of 1 learner" (TDD — failing test first, then fix)
- [ ] (user) Decide whether to add a housekeeping command to sweep orphaned report PDFs — the dev database holds 508 PDFs and 187 empty directories against 13 report rows, all debris from runs predating the deletion fix; this run's deletes (single, bulk and cohort-cascade) all removed their files correctly, so there is no live leak
- [ ] (cmd) Fix the plan's own contradiction: QA 2.9 still specifies the running footer as `{site} · Cohort progress report · {cohort}` "and nothing else", while QA 2.1 (updated for organisations) specifies `<site> · <organisation> · Cohort progress report · <cohort>` — the build matches QA 2.1
