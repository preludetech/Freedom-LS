...
- [x] (user) Resolve structure concern: course_applications → role_based_permissions / student_management reviewer edges (Task B.3) — **MOVED to `spec_dd/0. drafts/application-review-ui`.** The reviewer role grant (and its edges) left this spec when the whole review workflow was split out; the create-only base no longer introduces these edges. The resolution (Option 1: `assign_object_role` role machinery, grant on submit to site staff holding the reviewer role, `# TODO` for per-course scoping, regenerate `docs/app_structure.md`) now belongs to the review spec.
