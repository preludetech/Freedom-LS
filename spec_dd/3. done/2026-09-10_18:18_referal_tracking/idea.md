...
and advert in the first place. The tally is what makes the attribution rows mean anything: without it
you know a campaign produced eleven signups but not whether that came off a hundred arrivals or ten
thousand.

Arrivals are identified by an advert code, an opaque identifier like `P5H2B3C8` that a landing URL
carries to name one advert. Alongside it the row carries the standard campaign parameters and click
identifiers, so paid traffic carrying no advert code of ours is still attributable.

## First touch, and only first touch

The values are frozen at the visitor's first landing and never overwritten. Someone who arrives on
one campaign in January and another in February keeps January's row. Advert, campaign and landing
...
that expiry is what stops one campaign owning a visitor forever. It is not a separate thing to build.
...
Nothing on the landing path reads the database either. The only database work a landing does is the
tally increment when a cookie is minted, and that happens on the requests that mint one.
...
Frozen at first landing: the advert code, `utm_source`, `utm_medium`, `utm_campaign`, `utm_content`,
`utm_term`, `gclid`, `gbraid`, `wbraid`, `fbclid`, the landing path, the `Referer`, the raw query
string, and the first-seen timestamp.
...
`strict-origin-when-cross-origin`, so a genuine external referral yields the referring site's origin
and nothing more. Good for confirming that a link was placed where it was meant to be. Worthless as a
primary signal.

**The raw query string is stored once**, truncated, instead of a raw copy of every parsed field. It
is the only way to work out later why someone's link builder produced something unparseable.
...
whether by capping the distinct new keys minted per site per day, by folding everything past the cap
into a single overflow row, or some other way. The spec has to pick one.
...
There is no report and no dashboard. The changelist and its export are what the operator gets, which
makes the filters the product rather than a detail. The two questions people actually ask are "how
many signups came from this campaign last month" and "which advert brought them". So the changelist
needs a date hierarchy on first-seen, filters on source, medium and campaign, and search across the
learner's email, the campaign and the advert code. Click identifiers, cookie values, IP and user
agent are near-unique per visitor. They belong on the detail view, never in a filter.
...
the advert code are all typed by whoever crafted the URL. This is the one consumer this feature has,
and the values reaching it come from strangers. FLS has no existing export to inherit escaping from.
...
None of these is dropped. Each gets its own spec.

**Referrer codes for partner organisations.** A code issued to a partner so their traffic is
identifiable, held on `Organisation` and checked at landing. Deferred whole: no field, no validation,
no lookup on the capture path, and nothing in the row that points at one. Until it lands, partner
traffic is only as identifiable as the `utm_source` on the partner's link, which anyone can type, so
a partner's claim that they sent the traffic still cannot be settled. Two decisions already made
should survive into that spec. The row has to store both the code it saw and the `Organisation` it
resolved to, because a code held in a field rather than its own table can be cleared or reassigned
and would otherwise rewrite history. And an individual referrer needs an `Organisation` row, since
that is where the code lives, so issuing a code to a person is no lighter an act than issuing one to
a company. `research_referral_codes.md` has the alphabet, the argument against reusing `slug`, and
the code lifecycle.
...
- `research_referral_codes.md` covers code alphabets, why the slug is the wrong field, code
  lifecycle, and which abuse concerns evaporate without a reward. It is for the deferred referrer
  code work, not for this one.
