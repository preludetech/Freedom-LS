...
- [ ] (user + cmd) Fix QA bug: JSON-LD blocks use `type="application/json"` instead of `application/ld+json` (TDD — failing test first, then fix)
- [ ] (user + cmd) Fix QA bug: `sitemap.xml` URLs drop the port / don't use the current host (TDD — failing test first, then fix)
