...
- [x] (user + cmd) Fix QA bug: Tab click via HTMX strips panel chrome from inner DataTablePanels (TDD — failing test first, then fix)
- [x] (user + cmd) Fix QA bug: Multi-line `{# ... #}` comment leaks into rendered course progress page (TDD — failing test first, then fix)
