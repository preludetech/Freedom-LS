...
| `commands/sdd/update_claude_plugin_fls_content.md` | `fls:sdd-worker`, `fls:sdd-mechanic` | Entirely about syncing the sibling `fls-content/` course-author plugin — content schema, cotton templates, widget allowlists, `demo_content/` | same helper path | **FLS** — not a generic SDD step at all |
