#!/usr/bin/env python3
"""
First Class — cohort progress report (sample/template PDF).
...
PARTNER = "AeroVista Flight Academy · SACAA ATO 0231"
...
COURSES = [
    {
        "id": "rpc",
        "title": "RPC theory (multirotor)",
        "items": RPC_ITEMS,
        "quizzes": ["AL", "MET", "POF", "NAV"],
    },
    {
        "id": "radio",
        "title": "Radio licence primer",
        "items": RADIO_ITEMS,
        "quizzes": ["PHR", "PROC"],
    },
]
...
Q = {
    ("AL", 2): {
        "text": "Maximum height above the surface for a standard RPAS operation",
        "opts": {"A": "200 ft", "B": "400 ft", "C": "500 ft", "D": "1 000 ft"},
        "correct": "B",
    },
    ("AL", 5): {
        "text": "Minimum lateral distance from a person or structure not under the pilot's control",
        "opts": {"A": "30 m", "B": "50 m", "C": "100 m", "D": "150 m"},
        "correct": "B",
    },
    ("AL", 7): {
        "text": "Distance from an aerodrome within which RPAS operations require approval",
        "opts": {"A": "5 km", "B": "8 km", "C": "10 km", "D": "15 km"},
        "correct": "C",
    },
    ("AL", 9): {
        "text": "Final responsibility for the safe outcome of an RPAS flight rests with",
        "opts": {
            "A": "The observer",
            "B": "The pilot in command",
            "C": "The ATO",
            "D": "The operator's safety officer",
        },
        "correct": "B",
    },
    ("AL", 11): {
        "text": "RPAS operations at night are permitted",
        "opts": {
            "A": "Never",
            "B": "With the required approval in the operator's OpsSpec",
            "C": "Whenever VLOS can be maintained",
            "D": "Only with anti-collision lighting fitted",
        },
        "correct": "B",
    },
    ("MET", 1): {
        "text": "Conditions that most favour the formation of radiation fog",
        "opts": {
            "A": "Clear skies and light wind",
            "B": "Overcast skies and strong wind",
            "C": "Warm, gusty afternoons",
            "D": "The passage of a cold front",
        },
        "correct": "A",
    },
    ("MET", 3): {
        "text": "Density altitude increases when",
        "opts": {
            "A": "Air temperature rises",
            "B": "Air temperature falls",
            "C": "Surface pressure rises",
            "D": "Humidity falls",
        },
        "correct": "A",
    },
    ("MET", 6): {
        "text": "Wind direction in a METAR is given relative to",
        "opts": {
            "A": "True north",
            "B": "Magnetic north",
            "C": "The runway heading",
            "D": "Grid north",
        },
        "correct": "A",
    },
    ("MET", 8): {
        "text": "Cloud base heights in a METAR are reported",
        "opts": {
            "A": "Above ground level",
            "B": "Above mean sea level",
            "C": "As flight levels",
            "D": "Above the highest obstacle",
        },
        "correct": "A",
    },
    ("POF", 2): {
        "text": "Increasing the angle of attack beyond the critical angle causes",
        "opts": {
            "A": "Lift to keep increasing",
            "B": "Airflow to separate and the surface to stall",
            "C": "Drag to fall sharply",
            "D": "The lift-to-drag ratio to peak",
        },
        "correct": "B",
    },
    ("POF", 4): {
        "text": "In a steady hover, total rotor thrust is",
        "opts": {
            "A": "Greater than the aircraft's weight",
            "B": "Equal to the aircraft's weight",
            "C": "Less than the aircraft's weight",
            "D": "Independent of the aircraft's weight",
        },
        "correct": "B",
    },
    ("POF", 7): {
        "text": "Adding payload to a multirotor in the hover",
        "opts": {
            "A": "Reduces the power required",
            "B": "Has no effect on the power required",
            "C": "Increases the power required",
            "D": "Only affects endurance, not power",
        },
        "correct": "C",
    },
    ("NAV", 3): {
        "text": "With QNH set, the altimeter indicates",
        "opts": {
    ...
    ("NAV", 6): {
        "text": "On a South African chart, an area prefixed FAR is",
        "opts": {
            "A": "A danger area",
            "B": "A prohibited area",
            "C": "A restricted area",
    ...
    ("NAV", 7): {
        "text": "A CTR is best described as",
        "opts": {
            "A": "Uncontrolled airspace around any airfield",
            "B": "Controlled airspace around an aerodrome, from the surface up",
            "C": "A military flying corridor",
            "D": "Airspace above FL195",
        },
        "correct": "B",
    },
    ("PHR", 4): {
        "text": "The word WILCO means",
        "opts": {
            "A": "Message received",
            "B": "I understand and will comply",
            "C": "Say again",
    ...
    ("PHR", 5): {
        "text": "Readability is reported on a scale of",
        "opts": {"A": "1 to 3", "B": "1 to 5", "C": "1 to 9", "D": "A to E"},
        "correct": "B",
    },
    ("PROC", 1): {
        "text": "The VHF emergency frequency is",
        "opts": {
            "A": "118.10 MHz",
            "B": "121.50 MHz",
            "C": "124.80 MHz",
    ...
    ("PROC", 5): {
        "text": "Before transmitting on a frequency, first",
        "opts": {
            "A": "Increase the squelch",
            "B": "Transmit a radio check",
            "C": "Listen out to confirm the frequency is clear",
            "D": "Announce your callsign twice",
        },
        "correct": "C",
    },
}
...
STUDENTS = [
    ...
    dict(
        ...
        sid="FC-26-0391",
        start=d(5, 18),
        done={"rpc": 24, "radio": 10},
        ends={"rpc": d(8, 5), "radio": d(8, 8)},
        attempts={
            "AL": [(d(6, 5), {7: "A"})],
            "MET": [(d(6, 24), {})],
            "POF": [(d(7, 10), {})],
    ...
    dict(
        ...
        ends={"rpc": d(7, 28), "radio": d(7, 22)},
        attempts={
            "AL": [
                (d(7, 10), {2: "D", 5: "A", 7: "B", 9: "A", 11: "C"}),
    ...
    dict(
        ...
        ends={"rpc": d(8, 4), "radio": d(8, 6)},
        attempts={
            "AL": [(d(7, 6), {2: "D", 7: "B", 9: "C"})],
            "MET": [
                (d(7, 30), {1: "B", 3: "C", 6: "B"}),
                (d(8, 6), {1: "C", 3: "B", 6: "B", 8: "B"}),
            ],
        },
    ),
    dict(
...
def fmt(dt: date | None, year=True) -> str:
    if not dt:
        return "—"
...
for s in STUDENTS:
    ...
    for c in COURSES:
        ...
        if k:
            start = s["start"] + (
                timedelta(days=14) if c["id"] == "radio" else timedelta(0)
            )
...
class AtRiskRule:
    ...
    def evaluate(self, student: dict) -> str | None:
        return self.fn(student, **self.params)


def rule_no_activity(s: dict) -> str | None:
    if s["total_done"] == 0 and not s["scored"]:
        return (
            f"No recorded activity in any enrolled course since enrolment "
            f"on {fmt(s['start'])}."
        )
    return None


def rule_failed_latest(s: dict, pass_mark: int) -> str | None:
    failed = [(code, rec[2]) for code, rec in s["latest"].items() if rec[2] < pass_mark]
    if not failed:
        return None
    parts = [f"{QUIZZES[c]['title']} ({c}) — {sc}%" for c, sc in failed]
    return (
        f"Failed the latest attempt at {'; '.join(parts)} "
        f"against a {pass_mark}% pass mark."
    )


def rule_inactive(s: dict, days: int) -> str | None:
...
def cover() -> str:
    course_lines = "".join(
        ...
        f"{len(c['quizzes'])} quizzes</span></li>"
        for c in COURSES
    )
    return f"""
<section class="cover">
  <div class="cover-in">
    <div class="brandrow">
      <div><div class="wordmark">First Class</div><div class="tagline">Elevated.</div></div>
      <div class="accentline"></div>
    </div>
    <div class="cover-mid">
      <div class="overline">Training partner report</div>
      <div class="cover-title">Cohort progress report</div>
      <div class="cover-cohort">{esc(COHORT_NAME)}</div>
      <div class="cover-partner">Prepared for {esc(PARTNER)}</div>
  ...
  <div class="cover-band">
    <span class="band-brand">First Class</span>
...
def toc_row(href: str, label: str, level=1, cls="") -> str:
    return (
        f'<a class="toc-row lv{level} {cls}" href="{href}">'
...
def contents() -> str:
    rows = [
        toc_row("#glance", "Cohort at a glance"),
    ...
    rows += [
        toc_row(f"#stu-{s['id']}", f"{esc(s['last'])}, {esc(s['first'])}", 2)
        for s in STUDENTS
    ]
    rows.append(
        toc_row("#sec-confusions", "3&ensp;·&ensp;Quiz confusions across the cohort")
    )
    rows += [
        toc_row(f"#quiz-{code}", f"{esc(QUIZZES[code]['title'])} ({code})", 2)
        for code in QUIZZES
    ]
    ...
    legend_chips = "".join(
        ...
        for st, role, meaning in [
            ...
            (
                "error",
                "error",
                "significantly behind (1–19%), or a clear fail (below 60%)",
            ),
...
def summary_course_table(c: dict) -> str:
    ...
    group_th = "".join(
        f'<th colspan="2" class="qgroup c">{code}</th>' for code in codes
    )
    sub_th = "".join(
        '<th class="c sub qgroup">Score</th><th class="c sub">Att</th>' for _ in codes
    )
    body = []
    for s in STUDENTS:
        ...
        for code in codes:
            ...
            if rec:
                ...
                cells.append(
                    f'<td class="c qgroup st-{sst}">{gspan(sst)}&#8202;'
...
def wrong_rollup(s: dict, code: str) -> str:
    ...
    for qno in sorted(agg):
        ...
        picked = " · ".join(
            f"{L} — {esc(q['opts'][L])}"
            + (f' <span class="mono">×{n}</span>' if n > 1 else "")
...
def student_detail(s: dict, first: bool) -> str:
    ...
    if s["flags"]:
        top_sev = (
            "error" if any(r.severity == "error" for r, _ in s["flags"]) else "warning"
        )
        frows = "".join(
            f'<div class="reason">{badge(r)}<span>{esc(reason)}</span></div>'
            for r, reason in s["flags"]
        ...
        flags_html = (
            '<p class="noflags">No flags — no at-risk rule tripped at '
            "generation time.</p>"
        )
    ...
    if s["total_done"] == 0 and not s["scored"]:
        ...
        for c in COURSES:
            ...
            if done:
                ...
                items_html = (
                    f'<div class="subhead">Items completed</div>'
                    f'<table class="items"><tbody>{"".join(pairs)}</tbody></table>'
                )
            else:
                items_html = (
                    '<div class="subhead">Items completed</div>'
            ...
            if atts:
                att_html = (
                    f'<div class="subhead">Quiz attempts <span class="normal">'
                ...
                att_html = (
                    '<div class="subhead">Quiz attempts</div>'
            ...
            for code in c["quizzes"]:
                if s["scored"].get(code):
                    roll.append(
                        f'<div class="subhead">Incorrect answers — '
                        f"{esc(QUIZZES[code]['title'])} ({code}), all attempts</div>"
                        + wrong_rollup(s, code)
                    )
            blocks.append(
                f'<div class="courseblk">{head}{items_html}{att_html}{"".join(roll)}</div>'
            )
        body = "".join(blocks)
...
def confusions() -> str:
    ...
    for code, meta in QUIZZES.items():
        ...
        if not n_att:
            blocks.append(
                f'<div class="quizblk" id="quiz-{code}"><h2>{esc(meta["title"])} '
        ...
        if total_err_q > CONFUSION_CAP:
            cap_line = (
                f"Showing the worst {CONFUSION_CAP} of {total_err_q} questions with "
                f"at least one incorrect first attempt."
            )
        else:
            cap_line = (
                f"All {total_err_q} question{'s' if total_err_q != 1 else ''} with at "
...
@page {
  ...
  @bottom-left { content: "First Class · Cohort progress report · $footCohort";
...
<head>
<meta charset="utf-8"/>
<title>First Class — Cohort progress report · {esc(COHORT_CODE)}</title>
<meta name="author" content="First Class"/>
<meta name="description" content="Cohort progress report — sample/template render"/>
<style>{CSS_SRC.substitute(THEME | {"footCohort": COHORT_CODE})}</style>
</head>
<body>
{cover()}
{glance()}
