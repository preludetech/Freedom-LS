...
- [x] (cmd) Run `/fls-dev:do_qa` on `3d. report_smoke_qa/frontend_qa_report_smoke.md` — a smoke test of the cohort report feature against this branch's rewritten `gather.py`/`indexes.py`: one short report for RPAS Training, an organisation with a name and a logo. Checks that it generates, downloads under the organisation-first filename, opens, carries the brand, and — the part only this branch can break — that its quiz cells and completion percentages are not silently empty
...
- [x] (cmd) Use the `fls-dev:qa-data-helper` agent to create missing data for the Form branch of `content_tags.get_content_by_path` — no demo content links to a Form by path, so that branch has never been exercised — then re-run `/fls-dev:do_qa`. Done, but not via that agent: the branch needed authored content, not dev-DB rows, so `demo_content/functionality_demo_end_with_quiz/2. topic/content.md` now carries `<c-content-link path="../3. quiz/form.md">the Mid course Quiz</c-content-link>`, which resolves past the Topic lookup to the Form. Committed demo content ships downstream and survives `danger_content_delete`, where a QA fixture would not. `get_content_by_path` also gained unit coverage in `freedom_ls/content_engine/tests/test_content_tags.py` (it had none, on either branch) and `test_demo_content_form_link.py` guards the link against the silent rot that opened this gap. QA plan `3c` R6 was re-walked: R6.3 now passes
- [x] (cmd) Deferred the two `<c-content-link>` defects to their own spec, `spec_dd/1. next/content-links/`: a Form link renders `<a href="">` because `Form` has no `preview_url`, and `Topic.preview_url()` reverses `content_engine:topic_detail`, a URL name no URLconf defines, so a Topic link 500s. Nothing in FLS uses content links yet, and both fixes turn on a design question this branch has no business answering: content has no course-free URL, so a link can only resolve through `view_course_item` and its access, deadline and locking rules. The `TODO. - non-preview link` in `content-link.html:5` stays in place until that spec lands
- [x] (cmd) Fix the order-dependent flake in `freedom_ls/accounts/tests/test_allauth_signup_policy.py::test_is_open_for_signup_respects_force_site_name`. Under `pytest-randomly` it intermittently asserts `True is False` — `is_open_for_signup` falls back to the global `ALLOW_SIGN_UPS` instead of the ForcedSite policy — while passing in isolation, in deterministic order (`-p no:randomly`), and across 6/6 narrow runs. Confirmed pre-existing, not a regression from this branch: it fails on full-suite runs that exclude every file touched by the content-link work above
- [x] (cmd) Apply the `_base_manager` rule to `freedom_ls/accounts/checks.py:87`: `check_legal_docs_present_when_required` builds a by-site map with `SiteSignupPolicy.objects.select_related("site")` and then loops every Site, so under an ambient request the site-aware manager would narrow it to one policy and the check would mis-report every other site. Same family as the signup-policy flake fixed above, but out of its scope: system checks run at startup with no request, so nothing exercises it today. Done: the map is now built with `SiteSignupPolicy._base_manager`. The check's name in this item was wrong — no `check_terms_acceptance_has_document` exists — but line 87 was right. Not purely theoretical after all: under `mock_site_context` the check already ran with an ambient request, so the new `test_system_check_reads_every_sites_policy_when_another_site_is_ambient` in `test_legal_docs.py` failed on the old manager. `Site.objects` on the line above is Django's own `SiteManager` and was left alone
- [x] (cmd) Make `CurrentSiteMiddleware` (`freedom_ls/site_aware_models/middleware.py:8-13`) restore the previous `_thread_locals.request` instead of unconditionally `delattr`-ing it, and do it in a `try/finally`. Today a nested or re-entrant call leaves the thread with no ambient request rather than the one it had, which is the class of leak that made the signup-policy flake possible. Deliberately excluded from that fix: no test there covers it, and wrapping the current `delattr` in a `finally` would make the wrong teardown reliable rather than fixing it. Done: the previous request is captured and restored in a `finally`, falling back to `delattr` only when the thread held nothing to begin with — the idiom already used by `mock_site_context` and `qa_create_organisation_scenarios`. The `finally` also closes the second half of the leak: a view that raised used to strand its own request on the thread. `freedom_ls/site_aware_models/tests/test_middleware.py` is new and holds the first tests this middleware has had, one per case
- [x]WONTFIX  (user + cmd) Fix QA bug: cohort report counts a twice-placed form as complete at both placements, over-reporting completion and falsely marking a course complete — `_completed_items()` (`freedom_ls/reports/gather.py:293`) keys form completion on the Form pk, not the ContentCollectionItem, so one sitting ticks every placement of that form. Verified against three separately generated reports after `recalculate_progress_percentages`, so it is not stale data. Needs a product decision first: should the two KC/KC-2 columns become genuinely independent quiz columns with independent scores and verdicts, and what should happen to attempts whose `collection_item` is NULL (reachable — placement deletion is SET_NULL). See `3a. seam_qa/qa_report.md` bug B1 (TDD — failing test first, then fix)
