...
Delegate to `fls:sdd-mechanic`: have it run the steps in `fls-claude-plugin/commands/sdd/protected/move_spec_to_in_progress.md`, passing through the spec directory from Step 1.
...
Delegate to `fls:sdd-mechanic`: have it run the steps in `fls-claude-plugin/commands/sdd/protected/start_worktree.md`, passing through the spec directory (now under `spec_dd/2. in progress/`).
