...
3. Validate that `<PLUGINS_ROOT>/claude_plugins/sdd-claude-plugin/` exists — this confirms the `sdd`
...
2. If `claude.sh` exists, look for a `--plugin-dir` line pointing at `claude_plugins/sdd-claude-plugin`.
   ...
   ```bash
     --plugin-dir "$SCRIPT_DIR/$PLUGINS_ROOT/claude_plugins/sdd-claude-plugin" \
   ```
...
5. Confirm `<PLUGINS_ROOT>/claude_plugins/sdd-claude-plugin/` exists.
