...
Act like a human QA expert. Execute the given test plan. This command runs at **depth 0**, so its `fls-dev:qa-data-helper` delegation and the Step 7 desktop-testing exploration spawn are legal. See the `claude-code-authoring` skill for the model behind this.
...
**3e. Any `Agent` spawn is always a solo call.** This applies to all agents: `fls-dev:qa-data-helper`,
`sdd:sdd-worker`, `sdd:sdd-mechanic`, and `fls-dev:qa-bugfixer`. Never batch an `Agent` call with
a `Bash` or Playwright call in the same turn. The `fls-dev:qa-bugfixer` spawn (Step 14) is a
...
`.claude/fls-dev/scripts/qa_cleanup.sh`
...
`sdd:sdd-worker`** to explore it (non-interactive; it returns a structured `status`/`reason`). This
...
`fls-dev:qa-data-helper` rather than creating the data yourself or skipping the test. **This spawn is
also a solo call.**
...
Spawn a **solo `sdd:sdd-mechanic`** (Haiku) to run the compression. This is a **solo** `Agent` call
...
Spawn a **single solo `sdd:sdd-worker`** (Sonnet) to render the report. This is a **solo** `Agent`
...
Spawn **`fls-dev:qa-bugfixer`** as a **solo** `Agent` call (Rule 3e — CRITICAL). Never batch this
...
Spawn a **solo `sdd:sdd-mechanic`** (Haiku) to apply the todo ticks and additions. This is a
...
`claude_plugins/sdd/commands/protected/update_todo.md` and follow its steps literally. Pass the
