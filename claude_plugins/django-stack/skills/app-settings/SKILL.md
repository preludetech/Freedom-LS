---
name: app-settings
description: Define and manage per-app Django settings via per-app config.py modules. Use when adding, reading, or enforcing a setting an app owns, when a downstream project must supply a value, or when the user mentions config.py, AppSettings, declared_settings, or required settings.
...
This convention applies to any Django app that reads a project-level setting.
...
- **User mentions** `config.py`, `AppSettings`, `declared_settings`, `Setting`, or
  required settings.

## The shared base

A single shared module (e.g. `myproject/base/app_settings.py`) provides `AppSettings`,
`Setting`, and `required_settings_errors()`. Any app imports these from there. That module
must import only `django.conf`, `django.core.checks`, `django.core.exceptions`, and stdlib —
**no models** — so `from <app>.config import config` is safe at the top of any module (it
can't cause an import cycle).
...
```python
# myproject/search/config.py
from __future__ import annotations

from myproject.base.app_settings import AppSettings, Setting


class SearchConfig(AppSettings):
    SEARCH_BACKEND: str

    declared_settings = {
        "SEARCH_BACKEND": Setting(required=True),
    }
...
even though something else reads the value (a third party reads it):

```python
    declared_settings = {
        "SEARCH_CONFIG_VALIDATOR": Setting(default=None),
        # No safe empty default: the consumer reads registry["default"].
        "ADMONITION_TYPES": Setting(required=True),
        # Declared here only to record ownership; a third party reads it itself.
...
```python
from myproject.search.config import config

backend = import_string(config.SEARCH_BACKEND)
```
...
from myproject.base.app_settings import required_settings_errors
