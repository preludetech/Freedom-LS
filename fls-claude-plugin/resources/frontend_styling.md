...
The `@theme {}` block declares every `--color-*` custom property; Tailwind v4 generates the matching utility classes (`bg-<role>`, `text-<role>`, `border-<role>`, etc.) automatically.
