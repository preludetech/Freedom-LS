...
description: Subagent wrapper for `/plan_structure_review` invoked by `/plan_dev`. Reads the corresponding command file in full and follows its instructions, emitting a structured findings report instead of editing the plan.
...
1. Read `${CLAUDE_PLUGIN_ROOT}/commands/sdd/plan_structure_review.md` in full.
...
When the command file's "Step 6: Deliver the report" section describes two delivery contexts, you are the **wrapper-agent** context — emit the report as text, do not write `plan_structure_findings.md`. Your tool grants prevent you from writing files anyway, but state it clearly so you don't try.
