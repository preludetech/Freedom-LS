...
description: |-
  ...
  in its prompt (temp-then-rename) and returns a structured status. Never asks the user.
...
in the prompt — do not rename it or write anywhere else.

Write **atomically**: write `<path>.tmp` first, then rename it to `<path>`. This guarantees a crash
