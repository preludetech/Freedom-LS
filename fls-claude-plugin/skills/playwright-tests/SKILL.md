...
- `@pytest.mark.playwright` (all under `tests/e2e/`) is the **browser set a downstream excludes** — keep every browser test under this marker and don't rename it. For why, and the exact downstream selection, see the Portability note in the resource file and the `fls:testing` skill's marker taxonomy.
